...
def create_snapshot(ranking_a, ranking_b, start_with="A", exclude_item_ids=None):
...
def refresh_session(session, ranking_a, ranking_b, store, ttl_seconds=60, now=None):
    """
    ...
    new_snapshot = create_snapshot(
        ...
        ranking_b=ranking_b,
        start_with="A",
        exclude_item_ids=session.served_item_ids,
    )
...
def get_page_by_cursor(session, cursor, limit):
    ...
    for item in page_items:
        session.served_item_ids.add(item.item_id)
