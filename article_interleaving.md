...
```python
def balanced_interleaving(ranking_a, ranking_b, start_with="A", exclude_item_ids=None):
    if start_with not in ("A", "B"):
        raise ValueError("start_with must be 'A' or 'B'")
...
`exclude_item_ids`という要素をつけることで、以前interleavingされて出力されたランキングは出ないようになっている。

このようにして今回の本題であった以下の要件を満たすことができた。
- ランキングデータが800件以上ある
- 何ページにも分かれる
- ページをまたいでの重複表示は絶対に許されない
- A/Bのランキングを混ぜたい

SNSなどで無限スクロールなどもこのような同様なsnapshotとcursorを組み合わせているようなケースは多々あると思います。
これらをうまく活用してページネーションの表示について反映し、システムの評価に活用していきたいです。

## 今回のコードがあるリポジトリ
- https://github.com/saitoou/try-interleaving-prac
